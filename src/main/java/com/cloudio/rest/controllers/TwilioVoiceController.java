...
public class TwilioVoiceController {
    ...
    public Mono<String> handleTimeout(@RequestParam("adapterNumber") final String adapterNumber,
        ...
        final String newAdapterNumber = "+" + adapterNumber.trim();
        return redisOperations.opsForValue().get(newAdapterNumber)
                .flatMap(companySetting -> ringType.equals("IN_ORDER") ? twilioService.handleOneByOneTimeout(newAdapterNumber, nextIndex, companySetting)
                        : twilioService.handleVoiceMessage(newAdapterNumber, companySetting))
